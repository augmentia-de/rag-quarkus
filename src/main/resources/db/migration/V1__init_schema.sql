...
CREATE INDEX idx_chunk_doc_id ON rag_chunks(doc_id);
CREATE INDEX idx_chunk_title ON rag_chunks(title);
CREATE INDEX idx_chunk_embedding ON rag_chunks
    USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
CREATE INDEX idx_chunk_tsv ON rag_chunks USING gin(tsv);
...
CREATE INDEX idx_graph_nodes_chunk_id ON graph_nodes(chunk_id);
